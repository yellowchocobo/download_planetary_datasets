...
min_latitude = 27
max_latitude = 30
global_list = get_links_for_download(min_latitude, max_latitude)
...
TODO
-------
Modify it so it works. It does not work per today (12.06.2020)
"""
...
def get_all_repos_mini_rf():
    
    """
    returns all the ftp repo links of mini-rf. 
    ...
    -------
    repo_links : lst
        list of strings containings all the ftp repo links of mini-rf.
    """
    ...
    for i in range(len(default_link_part2)):
        ...
        for j in range(len(default_link_part3[i])):
            repo_links.append(default_link + default_link_part2[i] + default_link_part3[i][j] + '/level1/')
        
    return (repo_links)

#TODO not the best should do as with Kaguya's products
def get_links_for_download(min_latitude, max_latitude):
    
    """
    returns all the ftp links of mini-rf in between the min and max latitudes. 
    ...
    repos_mini_rf = get_all_repos_mini_rf()
